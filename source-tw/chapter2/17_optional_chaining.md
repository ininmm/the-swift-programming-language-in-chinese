...
* [可選鏈可替代強制解析](17_optional_chaining.md#optional_chaining_as_an_alternative_to_forced_unwrapping)
* [為可選鏈定義模型類](17_optional_chaining.md#defining_model_classes_for_optional_chaining)
* [通過可選鏈調用屬性](17_optional_chaining.md#calling_properties_through_optional_chaining)
* [通過可選鏈調用方法](17_optional_chaining.md#calling_methods_through_optional_chaining)
* [使用可選鏈調用子腳本](17_optional_chaining.md#calling_subscripts_through_optional_chaining)
* [連接多層鏈接](17_optional_chaining.md#linking_multiple_levels_of_chaining)
* [鏈接可選返回值的方法](17_optional_chaining.md#chaining_on_methods_with_optional_return_values)
...
最後，`Residence`定義了一個可選屬性叫`address`（`address?`）。`Address`類的屬性將在後面定義。 用於`rooms`數組的`Room`類是一個很簡單的類，它只有一個`name`屬性和一個設定`room`名的初始化器。
...
正如上面「 [可選鏈可替代強制解析](17_optional_chaining.md#optional_chaining_as_an_alternative_to_forced_unwrapping)」中所述，你可以利用可選鏈的可選值獲取屬性，並且檢查屬性是否獲取成功。然而，你不能使用可選鏈為屬性賦值。
...
如果你試圖通過可選鏈獲得`Int`值，不論使用了多少層鏈接返回的總是`Int?`。 相似的，如果你試圖通過可選鏈獲得`Int?`值，不論使用了多少層鏈接返回的總是`Int?`。
